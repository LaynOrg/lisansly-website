...
<template>
    <header class="bg-gradient-to-br from-gray-300 via-gray-100 to-white dark:from-black dark:via-gray-950
     dark:to-gray-900 border-solid border-b-2 border-gray-100 dark:border-gray-800">
        <UContainer class="flex items-center py-4 justify-between">
            <LuLogo />
            <div class="gap-3 hidden sm:flex">
                <UButton v-for="link in links" :key="link.href" :to="link.href" :aria-label="link.label"
                    :label="$t(link.label)" color="gray" variant="ghost" />
            </div>
            <div class="flex gap-3">
                ...
                <UButton class="sm:hidden" icon="i-heroicons-bars-3-20-solid" color="gray" variant="ghost" aria-label="Menu"
                    @click="isOpen = !isOpen" />
            </div>
        </UContainer>
    </header>
    <USlideover v-model="isOpen">
        <UCard class="flex flex-col flex-1"
            ...
            <div class="grid gap-3">
                <UButton v-for="link in links" :key="link.href" :to="link.href" :aria-label="link.label"
                    :label="$t(link.label)" color="gray" variant="ghost" size="lg" />
            </div>
        </UCard>
    </USlideover>
</template>